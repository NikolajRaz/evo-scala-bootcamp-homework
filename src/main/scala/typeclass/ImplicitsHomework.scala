...
object ImplicitsHomework {
  ...
  object SuperVipCollections4s {
    ...
    final class MutableBoundedCache[K: GetSizeScore, V: GetSizeScore](maxSizeScore: SizeScore) {
      ...
      def put(key: K, value: V): Unit ={
        if(map.keys.map(k => k.sizeScore).sum + map.values.map(v => v.sizeScore).sum + key.sizeScore + value.sizeScore > maxSizeScore ) {
          if(map.nonEmpty) {
            map -= map.head._1
            put(key, value)
          }
    ...
    object instances {
      ...
      implicit def stringGetSizeScore: GetSizeScore[String] = (v: String) => 12 + v.length * 2
      implicit def collectionGetSizeScore[T : GetSizeScore, Iterator[T] : Iterate]: GetSizeScore[Iterator[T]] = (v: Iterator[T]) => {
        implicitly[Iterate[Iterator]].iterator(v).map(v => v.sizeScore).sum + 12
      }
      implicit def coolestCollectionGetSizeScore[T : GetSizeScore, S : GetSizeScore,  Iterator[T, S] : Iterate2]: GetSizeScore[Iterator[T, S]] = (v: Iterator[T, S]) =>
        implicitly[Iterate2[Iterator]].iterator1(v).map(v => v.sizeScore).sum + implicitly[Iterate2[Iterator]].iterator2(v).map(v => v.sizeScore).sum + 12
    }
  }
  ...
  object MyTwitter {
    ...
    implicit def twitGetSizeScore: GetSizeScore[Twit] = (v: Twit) => v.id.sizeScore + v.userId.sizeScore + v.hashTags.sizeScore + v.attributes.sizeScore + v.fbiNotes.sizeScore

    implicit def fbiNoteGetSizeScore: GetSizeScore[FbiNote] = (v: FbiNote) => v.month.sizeScore + v.favouriteChar.sizeScore + v.watchedPewDiePieTimes.sizeScore
