...
object BinaryTreeSet {
  ...
  object Operation {
    ...
    final case class Insert(requester: ActorRef, id: Int, elem: Int)
        extends Operation

    // check whether an element `elem` is present in the tree
    final case class Contains(requester: ActorRef, id: Int, elem: Int)
        extends Operation

    // remove the element `elem` from the tree
    final case class Remove(requester: ActorRef, id: Int, elem: Int)
        extends Operation
  }
  ...
  object OperationReply {
    ...
    final case class ContainsResult(id: Int, result: Boolean)
...
final class BinaryTreeSet extends Actor {
  ...
  private def createRoot: ActorRef =
